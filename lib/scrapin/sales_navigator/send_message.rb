...
module ScrapIn
  module SalesNavigator
    class SendMessage
      PROFILE_MATCHER =   %r{https://(www\.|)linkedin\.com/sales/(people|profile)/.*}.freeze
      THREAD_MATCHER  =   %r{https://(www\.|)linkedin\.com/sales/inbox/.*}.freeze
      ...
      def initialize(session, thread_or_profile, message)
        if thread_or_profile.match(PROFILE_MATCHER)
          ...
          raise ArgumentError, 'This url is not a sales nav thread or profile url'
        end
        @session = session
        @message = message
      end
